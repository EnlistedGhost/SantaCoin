...
#if defined(NDEBUG)
# error "SantaCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "SantaCoin Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+13) - GetDepthInMainChain());
}
...
int64_t GetProofOfWorkReward(int64_t nHeight, int64_t nFees)
{
    ...
    else if(nHeight > 15)
    {
        ...
        if(randreward() <= 10000) // 10% Chance of superblock
            nSubsidy = 15 * COIN;
        else if(randreward() <= 4000) // 4% Chance of superblock
            nSubsidy = 50 * COIN;
        else if(randreward() <= 1000) // 1% Chance of superblock
            nSubsidy = 100 * COIN;
    }
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD_1 * 33 / (365 * 33 + 8);

    if(randreward() <= 10000) // 10% Chance of superblock (Fixed)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD_2 * 33 / (365 * 33 + 8);
    else if(randreward() <= 4000) // 4% Chance of superblock (Fixed)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD_3 * 33 / (365 * 33 + 8);
    else if(randreward() <= 1000) // 1% Chance of superblock (Fixed)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD_4 * 33 / (365 * 33 + 8);
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Modified & revised by bitbandi for PoW support
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    //LogPrintf("SantaCoin "we know de wey" retargetted using: DGW-v3 difficulty algo \n");
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    if ((pindexNew->nHeight % 20180) == 0 || (!fIsInitialDownload && (pindexNew->nHeight % 144) == 0))
    {
...
{
    RenameThread("SantaCoin-loadblk");
